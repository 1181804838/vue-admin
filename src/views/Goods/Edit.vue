			...
			<el-form :model="ruleForm" ref="ruleForm" label-width="120px" class="demo-ruleForm">
				...
				<el-form-item label="商品轮播图" class="el-form--label-left">
						<el-upload action="/api/upload/slider" :headers="headers" :file-list="sliderarr" list-type="picture-card" :on-preview="handlePictureCardPreview"
						 :on-remove="handleRemoveSlider" :on-success="handleSuccessSlider">
				...
				<el-form-item label="商品描述" class="el-form--label-left">
					<div ref="editor" style="text-align:left" class="toolbar" v-model="ruleForm.detail"></div>
				</el-form-item>
...
<script>
	...
	export default {
		...
		async mounted() {
			...
			let goodinfo = await this.loadgood(this.id);
			this.ruleForm = goodinfo;
			this.ruleForm.slider = this.ruleForm.slider.split(",");
			this.ruleForm.slider.forEach((item, index) => {
				if (item != "") {
					this.sliderarr[index] = {
						url: item
					}
					this.$set(this.sliderarr, index, this.sliderarr[index]);
				}
			});
			...
			editor.create();
			editor.txt.html(goodinfo.detail);
		},
		methods: {
			async loadcity(id){
			...
			//获取指定id商品
			async loadgood(id) {
				let { status, data } = await Goods.good({ id });
				if (status) {
					return data;
				}
			},
			...
			async handleEdit() {
				let { status } = await Goods.edit({ ...this.ruleForm });
				if (status) {
